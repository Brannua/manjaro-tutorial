...
尽可能地只使用 pacman 下载安装软件包，因为 pacman 下载的软件包都是经过了官方测试的软件包，比较有保障

若 pacman 发现官方的软件仓库中没有我们需要的软件包，那么可以谨慎地从 [AUR](https://aur.archlinux.org/) 中寻找你需要的软件包，然后使用第三方软件包管理器 [yay](https://github.com/Jguer/yay) 进行下载安装

> 注意：AUR 是 ArchUser Repository，任何人都可以向其中发布软件包，其中的软件包都是未经官方验证的，因此应谨慎使用其中的软件包，比如当你从 AUR 中发现了你需要的软件包，在决定下载安装该软件包之前，你需要先做下面的几步

1. 查看该软件包在 AUR 中的详细信息，尤其关注该软件包发布者是否在积极维护该软件包，并查看评论区的用户评论看有没有人反映该软件包有问题

2. 查看该软件包的官方地址，比如该软件包的 github 地址，查看其 star 数量和 issues 活跃度，确保该软件包的可用性
